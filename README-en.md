...
- [@react-native-community/geolocation](https://github.com/michalchudziak/react-native-geolocation) + [expo-location](https://docs.expo.dev/versions/latest/sdk/location/) — device location; non-GMS devices fall back to the system LocationManager, with GPS cache and IP-based fallbacks
...
- Homework and courseware connect directly to `courses.zju.edu.cn` (学在浙大 / TronClass)
...
- Live weather: auto-locates (system location → GPS cache → IP fallback, works on non-GMS devices), shows temperature range, rain probability, and contextual tips; switches to tomorrow's forecast after 9 PM
...
│
├── lib/
│   ├── zju-client.ts              # ZJU client entry (re-exports lib/zju/)
│   ├── zju/                       # CAS auth + 教务 / TronClass data requests (no server)
...
- **Slow first fix on non-GMS Android** — devices without Google Play Services locate via the system LocationManager, which can be slow on cold start; on timeout the app automatically falls back to IP-based location

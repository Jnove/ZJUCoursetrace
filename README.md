...
**课程表**
- 周网格视图与日列表视图，一键切换
- 学期选择器按学年分组，默认定位当前学期（假期自动指向下一学期）
- 单 / 双周筛选
- 一键截图并分享或保存到相册
- 自定义课程：手动添加实验课 / 社团等日程，与正式课表一起显示（仅存本机）
- 导出课表为 .ics 日历文件（自动处理单双周，可多选学期一起导出）：Android 直接调起系统日历导入，iOS 走分享面板
...
**通知与设置**
- 登录后显示学生姓名（首页欢迎语与设置页头像），自动从统一身份认证信息页解析并本地缓存
- Android 状态栏实时显示当前课程或下节课及倒计时，静默常驻不打扰
- 出分提醒：后台定期检查成绩（约每 2 小时），有新成绩发布时推送通知
- 作业 / 考试提醒：截止或开考前 24 小时和 2 小时各提醒一次，可在设置中关闭
- 深色 / 浅色 / 跟随系统主题，另有 AMOLED 纯黑深色档（OLED 屏省电）
- 个性化：15 种界面主题色、3 种圆角样式、5 套课程色彩方案，含实时预览
- 应用内更新检测：Android 支持直接下载安装 APK（目前这需要“安装未知应用”权限，这可能导致一些不必要的安全问题，所以我们推荐到releases下载），iOS 跳转 GitHub Releases
...
│   ├── course-notification.ts     # 课程常驻通知（前台每秒 / 后台 background-task 定期刷新）
│   ├── _core/shadow.ts            # 随深浅色自适应的卡片阴影（深色模式改用描边）

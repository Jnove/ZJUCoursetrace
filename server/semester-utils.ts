...
export function getCurrentSemester(date: Date = new Date()): SemesterInfo | null {
  ...
  const nextYearLantern = getLanternFestivalDate(year + 1);
  ...
  const nextSpringWeek1 = getMonday(nextYearLantern);
 ...
 * 测试函数
 */
export function testSemesterDetection() {
  const testDates = [
    new Date(2026, 4, 21),  // 2026-05-21
    new Date(2025, 8, 15),  // 2025-09-15
    new Date(2026, 0, 1),   // 2026-01-01
    new Date(2026, 1, 15),  // 2026-02-15
  ];
  ...
  for (const date of testDates) {
    ...
    if (semester) {
      console.log(`  学年: ${semester.schoolYear}`);
      console.log(`  学期: ${semester.semester}`);
      console.log(`  周次: 第${semester.week}周`);
    } else {
      console.log('  不在任何学期内');
    }
    console.log('');
